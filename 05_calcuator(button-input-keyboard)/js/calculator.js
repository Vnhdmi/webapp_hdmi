...
function update(calcType) {
  ...
  // １つ前の入力が演算子（演算子 入力しなおし）
    ...
    if (wkFirst === "1") {
      ...
      // 演算子 入力しなおし
      const wkLogLastWord = elementcalcLog.innerHTML.slice(-1); //ログ最後の１文字
...
function dspResult() {
  if (wkFirst === "0" && wkBefore === "0") {
    elementcalcLog.innerHTML = elementcalcLog.innerHTML + Number(elementResult.innerHTML) //calc log
    calculator();
    wkCalc = "="; //save = 
    wkBefore = "1";
  }
